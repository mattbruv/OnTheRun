-- Cast Lingo BehaviorScript DriveCPU3[cont].ls
...
global G, havok, CPU3, trail

on beginSprite me
  debugTrail = new(script("trail"), 4, 1, 0, 0, 0, 1)
  CPU1 = me
  pSprite = sprite(me.spriteNum)
  world = pSprite.member
  pmodel = world.model("AutoCPU1")
  pPlayer = world.model("AutoPlayer")
  pTarget = world.model("Target3")
...
on aiLookForMarker me, mark
  ...
  repeat with i = 1 to radar.count
    name = radar[i].name
    if name contains "Trail2" or name contains "CPU" then
      nothing()
      next repeat
    end if
  ...
  if havok.simTime - aiLastStatus.time > 1 then
    if aiLastStatus.status = aiStatus and aiLastStatus.Action = aiAction then
      if aiStatus = #Ok and pCurrentSpeed < 1 then
        put "DEBUG:Loop detected - NewStatus = #retro"
        pGoingRight = 1 - pGoingRight
        pGoingLeft = 1 - pGoingLeft
        me.aiChangeStatus(#retro)
      end if
    end if
    me.aiSaveStatus()
  ...
  if pheight < pMaxHeight then
    ...
    if canTurn then
      if pGoingLeft > 0 then
        imp = currentAxis * (pTurnGain * mass) * pGoingLeft
        if pGoingBackward then
          imp = -imp
        end if
      ...
      if pGoingRight > 0 then
        imp = currentAxis * (-pTurnGain * mass) * pGoingRight
        if pGoingBackward then
          imp = -imp
        end if
        rb.applyAngularImpulse(imp)
    ...
    if canTurn = 0 or pGoingRight = 0 and pGoingLeft = 0 then
